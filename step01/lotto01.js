...
let lottos = [];
let luckyNumbers = [];
let win3 = 0 , win4 = 0, win5 = 0, win6 = 0;
const winPrices = {
  win3: 5000,
  win4: 50000,
  win5: 1500000,
...
const createLotto = () => {
  ...
  for (let i = 0; i < lottoLength; i++) {
    lotto.push(AllLottoNumbers.splice(generateRandomIndex(AllLottoNumbers.length), 1)[0]);
  }
...
const displayWinStatistics = () => {
  console.log(`
    ...
    나의 수익금은 ${(win3 * winPrices.win3 + win4 * winPrices.win4 + win5 * winPrices.win5 + win6 * winPrices.win6).toLocaleString()}원 입니다.`
  );
}
